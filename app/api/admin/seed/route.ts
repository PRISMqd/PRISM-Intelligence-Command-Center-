...
const GHOST_NOTES = [
  {
    inference_basis: 'No evidence or claims registered for competitor pricing in the PAC market segment. Decision D-7 assumed this without documentation.',
    confidence: 'high',
    confidence_score: 0.82,
  },
  {
    inference_basis: 'CMS reimbursement pathway referenced in 3 decisions but no formal claim or evidence object exists for it in the knowledge base.',
    confidence: 'medium',
    confidence_score: 0.61,
  },
  {
    inference_basis: 'Post-market surveillance workflow references a monitoring protocol not registered in the knowledge base.',
    confidence: 'low',
    confidence_score: 0.28,
  },
  {
    inference_basis: 'Patient adherence rate assumption (>85%) underpins the efficacy model but no primary data source has been linked.',
    confidence: 'medium',
...
export async function POST() {
  ...
  // Seed org health metrics
  const metricsToInsert = ORG_HEALTH_METRICS.map(m => ({
    metric_type: 'org_health' as const,
  ...
  const ghostObjects = GHOST_NOTES.map(g => ({
    object_type: 'ghost_note' as const,
  ...
  return NextResponse.json({
    ok: true,
    seeded: {
      metrics: ORG_HEALTH_METRICS.length,
      briefs: 1,
      ghost_notes: GHOST_NOTES.length,
    },
  })
}
